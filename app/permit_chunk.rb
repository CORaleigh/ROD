...
require_relative '../lib/helpers.rb' 
 ...
 DB = Sequel.oracle( :database => 'irisdev', :host => 'ucsdevrac1', :port => 1531, :user => configatron.user, :password => configatron.pass)
...
class ConnectQuery
  ...
  def initialize(db, min)
   ...
   :ignore_ssl => true 
    ...
    @db = db
    @view_id = 'iwpw-pyjf'   #permit data-set code for Socrata
    @payload =[]
  ...
  def export  #process the data and send to Socrata
   ...
   h.rewrite(:devplan_devplan_name => :development_plan_name,
    ...
    ##############concatenate address fields to Location 1       
        temp_address = {("location concat").to_sym =>(h[:street_num].to_s << ' ' << 
                                               h[:dir_pre].to_s << ' ' << 
                                               h[:street_name].to_s << ' ' << 
                                               h[:street_type].to_s << ' ' << 
                                               h[:dir_suf].to_s << ' ' << 
        ...
        puts package[:permit_num]
        @payload << package
    end
 puts @payload
 #    @client.post(@view_id, @payload)  #upload to Socrata
    
    LOGGER.info "Upload Successful for permit numbers '#{@min}' - '#{@max}'"  
